        ...
        public async Task<IActionResult> AddToCart(int sanId, string[] bookingDate, string[] startTime, string[] endTime)
        {
            try
            {
                ...
                for (int i = 0; i < bookingDate.Length; i++)
                {
                    ...
                    // ===================== CHECK THỜI GIAN THỰC =====================
                    var now = DateTime.Now;

                    bool isPast =
                        date.Date < now.Date ||  // ngày đã qua
                        (date.Date == now.Date && end <= now.TimeOfDay); // hôm nay nhưng đã qua giờ kết thúc

                    if (isPast)
                    {
                        TempData["Error"] =
                            $"Không thể đặt khung giờ đã qua ({date:dd/MM/yyyy} {start:hh\\:mm}-{end:hh\\:mm}).";
                        return RedirectToAction("Detail", "San", new { id = sanId, startDate = date.Date });
                    }
                    // =================================================================


                    // ============ CHECK THỜI GIAN HOẠT ĐỘNG ===============
                    ...
                    // =============== CHECK SLOT KHẢ DỤNG TRONG DB ===============
                    var slots = await _bookingSlotRepository
                        .GetAvailableSlotsBySanIdAndDateRangeAsync(sanId, date, start, end);

                    var requiredSlots = GenerateRequiredSlots(sanId, date, start, end);

                    if (slots.Count() != requiredSlots.Count() ||
                        !slots.All(s => requiredSlots.Any(rs =>
                            rs.StartTime == s.StartTime && rs.EndTime == s.EndTime)))
                    {
                        _logger.LogWarning($"Not all slots are available for San ID {sanId} on {date:dd/MM/yyyy}");
                    ...
                    // =============== CHECK TRÙNG TRONG CART =================
                    foreach (var slot in slots)
                    {
                        var existingItem = _bookingCart.Items
                            .FirstOrDefault(item => item.BookingSlotId == slot.Id);

                        if (existingItem != null)
                        {
                            TempData["Error"] =
                                $"Một hoặc nhiều slot đã có trong giỏ hàng cho ngày {date:dd/MM/yyyy}.";
                            return RedirectToAction("Detail", "San", new { id = sanId });
                        }
                    }
                    // ===========================================================
